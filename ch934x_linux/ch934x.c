...
static unsigned int ch934x_tty_write_room(struct tty_struct *tty)
{
	struct ch934x *ch934x = tty->driver_data;
	/*
...
static unsigned int ch934x_tty_chars_in_buffer(struct tty_struct *tty)
{
	struct ch934x *ch934x = tty->driver_data;
	/*
						...
						struct ktermios *termios_old)
{
	...
	if (C_BAUD(tty) == B0) {
		newline.dwDTERate = ch934x->line.dwDTERate;
		newctrl &= ~(CH934X_CTRL_DTR | CH934X_CTRL_RTS);
	} else if (termios_old && (termios_old->c_cflag & CBAUD) == B0) {
		newctrl |=  CH934X_CTRL_DTR | CH934X_CTRL_RTS;
	}

	if (memcmp(&ch934x->line, &newline, sizeof newline) == 0)
		return;
...
//		ch34x_set_control(ch934x, ch934x->ctrlout = newctrl);
	...
	if (memcmp(&ch934x->line, &newline, sizeof newline)) {
		memcpy(&ch934x->line, &newline, sizeof newline);
...
static int __init ch934x_init(void)
{
	...
	ch934x_tty_driver = tty_alloc_driver(CH934X_TTY_MINORS, 0);
	...
	if (retval) {
		tty_driver_kref_put(ch934x_tty_driver);
		return retval;
	}
